            ...
            <div id="main_content">
                ...
                <div class="center_content">
                    <%
                                ...
                                session.setAttribute("price", pricee);
                                String producere = request.getParameter("producer");
                                session.setAttribute("producer", producere);
                                String pride = session.getAttribute("prid").toString();
                                
                                String ratinge = request.getParameter("rating");
                                session.setAttribute("rating", ratinge);
                                String descriptione = request.getParameter("description");
                                session.setAttribute("description", descriptione);

                                String sellere = request.getParameter("seller");
                                session.setAttribute("seller", sellere);
                                String imge = request.getParameter("img1");
                                session.setAttribute("img1", imge);
                                String quantitye = request.getParameter("quantity");
                                session.setAttribute("quantity", quantitye);


                                out.print("<div class=\"center_title_bar\"> You Are About to Order:</div>"
                                        + "<div class=\"prod_box_big\">"
                                        + "<div class=\"center_prod_box_big\">"
                                        + "<div class=\"product_img_big\">"
                                        + "<img src=\"images/" + imge + "\" width=\"180\" height=\"200\" alt=\"\" title=\"\" border=\"0\" />");
                                ...
                                out.println("</div>"
                                        ...
                                        + " " + sellere + "</span><br />"
                                        + " Description :<span class=\"blue\">" + descriptione + "</span><br />"
                                        + "</div>"
                                        + "<div class=\"prod_price_big\"> <span class=\"price\">" + pricee + "$</span></div>"
                                        + "<h3><font color=\"Red\"><i>Are You Sure You Want To Buy This Item??</i></font><h3>"
                                        + " <form method= \"POST\" action=\"FinalConfirmation.jsp\" id=\"orderForm\">"
                                        + " Quantity :");
                    %>
                    <form method= "POST" action="FinalConfirmation.jsp" id="ConfirmForm">
                        <select id="quantity" name="selQuant">
                            <% for (int i = 1; i <= (Integer.parseInt(quantitye)); i++) {
